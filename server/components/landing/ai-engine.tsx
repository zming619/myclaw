...
import { BrainCircuit, Database, Search, MessageSquareQuote, CheckCircle2, ChevronRight, MessageSquareText, FileText, Send, Zap, AudioLines } from "lucide-react";
...
const ragFeatures = [
  { title: "知识库管理", desc: "支持创建多个知识库，按分类管理，设置关键词和权重。", icon: Database },
  { title: "文本向量化", desc: "支持向量匹配，支持text-embedding-v3和doubao多模态向量模型。", icon: FileText },
  { title: "语义检索", desc: "余弦相似度匹配，自动命中离中最相关的知识片段作为上下文注入AI对话。", icon: Search },
  { title: "微信客服融合", desc: "微信自动回复结合RAG知识库，让AI用你自己的业务知识精准回答客户问题。", icon: MessageSquareQuote },
];

const scenarios = [
  { name: "微信智能客服", icon: MessageSquareText },
  { name: "AI内容生成", icon: FileText },
  { name: "AI员工定时任务", icon: BrainCircuit },
  { name: "Function Calling", icon: Zap },
  { name: "智能群发文案", icon: Send },
  { name: "语音合成 TTS", icon: AudioLines },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              {models.map((model) => (
                <div key={model.name} className="flex items-center justify-between p-4 rounded-xl bg-white border border-border group hover:border-brand/40 transition-all cursor-default">
          ...
          {/* RAG Details */}
          <div className="p-10 rounded-3xl bg-navy text-white shadow-2xl space-y-10">
            <div>
