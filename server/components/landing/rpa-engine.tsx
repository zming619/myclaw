...
import { MessageCircle, PlayCircle, Bot } from "lucide-react";
import Image from "next/image";

const platforms = [
  { name: "抖音", icon: "/platforms/tiktok.svg", desc: "发布 · 互动 · 获客" },
  { name: "快手", icon: "/platforms/kuaishou.svg", desc: "发布 · 互动 · 获客" },
  { name: "小红书", icon: "/platforms/xiaohongshu.svg", desc: "发布 · 互动 · 获客" },
  { name: "微信", icon: "/platforms/wechat.svg", desc: "消息 · 朋友圈 · SOP" },
  { name: "视频号", icon: "/platforms/channels.svg", desc: "发布 · 登录" },
  { name: "B站", icon: "/platforms/bilibili.svg", desc: "视频发布" },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {platforms.map((platform) => (
              <div key={platform.name} className="flex flex-col items-center justify-center p-8 rounded-2xl bg-surface border border-border group hover:border-brand/40 hover:bg-white hover:shadow-lg transition-all">
                <div className="h-12 w-12 mb-4 bg-white rounded-xl shadow-sm flex items-center justify-center group-hover:scale-110 transition-transform">
                  {/* Using icons as placeholders for SVG logos */}
